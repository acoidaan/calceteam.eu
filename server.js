...
db.on("connection", function (connection) {
  console.log(
    "✅ Nueva conexión MySQL establecida como id " + connection.threadId
  );
});
...
// ==========================================
// CREAR TABLA REFRESH_TOKENS AL INICIO
// ==========================================
...
function generateFootballSchedule(teams, tournamentId, startDate) {
  ...
  console.log(
    `🏈 [DEBUG] Input - ${teams.length} equipos, torneo ${tournamentId}`
  );
  ...
  console.log(
    `🏈 [DEBUG] - Equipos array:`,
    teamsArray.map((t) => `${t.id || "NULL"}: ${t.name}`)
  );
  ...
  for (let jornada = 1; jornada <= totalTeams - 1; jornada++) {
    ...
    console.log(
      `🏈 [DEBUG] Jornada ${jornada} generó ${jornadaMatches.length} partidos`
    );
    addMatchesToList(
      matches,
      jornadaMatches,
  ...
  for (let jornada = 1; jornada <= totalTeams - 1; jornada++) {
    ...
    console.log(
      `🏈 [DEBUG] Generando jornada ${jornadaVuelta} (VUELTA de jornada ${jornada})`
    );
    const jornadaMatches = generateJornadaMatches(teamsArray, jornada, true);
    console.log(
      `🏈 [DEBUG] Jornada ${jornadaVuelta} generó ${jornadaMatches.length} partidos`
    );
    addMatchesToList(
      matches,
      jornadaMatches,
...
function generateJornadaMatches(teams, jornada, isVuelta) {
  console.log(
    `⚽ [DEBUG] === GENERANDO JORNADA ${jornada} (${
      isVuelta ? "VUELTA" : "IDA"
    }) ===`
  );
  console.log(
    `⚽ [DEBUG] Teams input:`,
    teams.map((t) => `${t.id || "NULL"}: ${t.name}`)
  );
  ...
  if (jornada % 2 === 1) {
    ...
    console.log(
      `⚽ [DEBUG] Equipo fijo: ${teams[0].name} vs ${teams[jornada].name}`
    );
  } else {
    homeTeams.push(teams[jornada]);
    awayTeams.push(teams[0]);
    console.log(
      `⚽ [DEBUG] Equipo fijo: ${teams[jornada].name} vs ${teams[0].name}`
    );
  }
  ...
  for (let i = 1; i < totalTeams / 2; i++) {
    ...
    const awayIndex =
      ((jornada - i - 1 + totalTeams - 1) % (totalTeams - 1)) + 1;

    console.log(
      `⚽ [DEBUG] Rotación ${i}: homeIndex=${homeIndex}, awayIndex=${awayIndex}`
    );
    ...
    if (homeIndex < teams.length && awayIndex < teams.length) {
      ...
      console.log(
        `⚽ [DEBUG] Añadido: ${teams[homeIndex].name} vs ${teams[awayIndex].name}`
      );
    } else {
      console.log(
        `⚽ [DEBUG] ⚠️ Índices fuera de rango: homeIndex=${homeIndex}, awayIndex=${awayIndex}, teams.length=${teams.length}`
      );
    }
  }

  console.log(
    `⚽ [DEBUG] Equipos locales:`,
    homeTeams.map((t) => t.name)
  );
  console.log(
    `⚽ [DEBUG] Equipos visitantes:`,
  ...
  for (let i = 0; i < homeTeams.length; i++) {
    ...
    console.log(
      `⚽ [DEBUG] Creando partido ${i + 1}: ${team1.name} vs ${
        team2.name
      } (antes de vuelta)`
    );
    ...
    if (isVuelta) {
      ...
      console.log(
        `⚽ [DEBUG] Partido ${i + 1} invertido (vuelta): ${team1.name} vs ${
          team2.name
        }`
      );
    }
    ...
    if (team1.id && team2.id) {
      ...
      console.log(
        `⚽ [DEBUG] ✅ Partido válido añadido: ID${team1.id} vs ID${team2.id}`
      );
    } else {
      console.log(
        `⚽ [DEBUG] ❌ Partido con descanso omitido: ${team1.name} vs ${team2.name}`
      );
    }
  }

  console.log(
    `⚽ [DEBUG] Jornada ${jornada} completada. Partidos válidos: ${matches.length}`
  );
  return matches;
}

// Agregar partidos a la lista con fecha y jornada con debug
function addMatchesToList(
  matches,
  jornadaMatches,
  tournamentId,
  startDate,
  ...
  console.log(
    `📅 [DEBUG] Añadiendo ${jornadaMatches.length} partidos a jornada ${jornada}`
  );
  ...
  console.log(
    `📅 [DEBUG] Fecha calculada para jornada ${jornada}: ${
...
function updateTournamentStats(tournamentId, callback) {
  console.log(
...
// ==========================================
// RUTAS DE AUTENTICACIÓN CON REFRESH TOKENS
// ==========================================
...
app.post("/api/login", (req, res) => {
  ...
  db.query(query, [email], async (err, results) => {
    ...
    db.query(insertRefreshToken, [user.id, refreshToken], (err) => {
      if (err) {
...
app.post("/api/refresh", (req, res) => {
  ...
  try {
    ...
    // Verificar que existe en la BD y no ha expirado
    const query = `
    ...
    db.query(query, [refreshToken], (err, results) => {
      ...
      // Actualizar refresh token en BD (rotación)
      const updateQuery = `
      ...
      db.query(updateQuery, [newRefreshToken, user.id], (err) => {
        if (err) {
          console.error("Error actualizando refresh token:", err);
          return res.status(500).json({ message: "Error interno" });
        }
...
app.post("/api/logout", verifyToken, (req, res) => {
  ...
  db.query(deleteQuery, [req.userId], (err) => {
    if (err) {
      console.error("Error eliminando refresh token:", err);
    }
...
app.get("/api/user/profile", verifyToken, (req, res) => {
  ...
  db.query(query, [req.userId], (err, results) => {
    ...
    if (results[0].profile_pic) {
      profilePic = `data:image/jpeg;base64,${results[0].profile_pic.toString(
        "base64"
      )}`;
    }
  ...
  (req, res) => {
    ...
    db.query(checkCreatorQuery, [teamId, req.userId], (err, teamResults) => {
      ...
      if (teamResults.length === 0) {
        return res
          .status(403)
          .json({ message: "Solo el creador puede editar el equipo" });
      }
...
app.post("/api/tournament/leave", verifyToken, (req, res) => {
  ...
  if (!tournamentId || !teamId) {
    return res
      .status(400)
      .json({ message: "tournamentId y teamId requeridos" });
  }
      ...
      db.query(getTournamentQuery, [tournamentId], (err, tournaments) => {
        ...
        regenerateMatches(tournamentId, tournament, (err, matchesCreated) => {
          if (err) {
            ...
            return res.json({
              message:
                "Saliste del torneo exitosamente, pero hubo un error regenerando el calendario",
            });
          }
...
app.get("/api/tournaments/my-tournaments", verifyToken, (req, res) => {
  ...
  db.query(query, [teamId], (err, results) => {
    ...
    console.log(
      `✅ Torneos encontrados para equipo ${teamId}:`,
      results.length
    );
    res.json({ tournaments: results });
  });
...
app.get("/api/matches/my-team-matches", verifyToken, (req, res) => {
  ...
  db.query(query, [teamId, teamId], (err, results) => {
    ...
    console.log(
      `✅ Partidos encontrados para equipo ${teamId}:`,
      results.length
    );
...
// Admin: Obtener todos los partidos
...
app.put("/api/admin/matches/:matchId", verifyToken, isAdmin, (req, res) => {
  ...
  if (match_date) {
    const mysqlDate = match_date.includes("T")
  ...
  values.push(matchId);
  const updateQuery = `UPDATE tournament_matches SET ${fieldsToUpdate.join(
    ", "
  )} WHERE id = ?`;
...
// Admin: Regenerar partidos manualmente
app.post(
...
app.post(
  "/api/admin/tournament/:id/update-stats",
  verifyToken,
  isAdmin,
  (req, res) => {
    const { id } = req.params;

    console.log(`📊 [ADMIN] Recalculando estadísticas para torneo ${id}`);

    updateTournamentStats(id, (err, teamsUpdated) => {
      if (err) {
        console.error("❌ [ADMIN] Error actualizando estadísticas:", err);
        return res.status(500).json({
